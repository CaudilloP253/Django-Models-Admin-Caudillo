...
class OrderManagerQuerySet(models.query.QuerySet):
    ...
    def not_created(self):
        return set.exclude(status="created")
    ...
    def by_weeks_range(self,weeks_ago, number_of_weeks):
        if number_of_weeks > weeks_ago:
            number_of_weeks = weeks_ago
        days_ago_start = weeks_ago * 7
        days_ago_end = days_ago_start * (number_of_weeks)
        start_date = timezone.now() - timedelta(days_ago_start)
...
class Order(models.Model):
    ...
    updated = models.DateTimeField() # se elimino auto_now=True 
